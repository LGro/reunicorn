...
Widget contactsListView(
  ...
  buildItemWidget: (contact) => CheckboxListTile(
    ...
    onChanged: (checked) =>
        updateCircleMembership(contact.coagContactId, checked ?? false),
  ),
...
List<Widget> detailsList(
  ...
  details
          .map(
            (label, value) => MapEntry<String, Widget>(
              label,
              CheckboxListTile(
                value:
                    getDetailSharingSettings(label)?.contains(circleId) ??
                    false,
                onChanged: (editCallback == null)
                    ? null
                    : (checked) => editCallback(label, checked ?? false),
                title: Row(
...
Widget _pictureSelection(BuildContext context, CircleDetailsState state) => Row(
  children: [
    ...
    const SizedBox(width: 8),
    Expanded(child: Text(context.loc.profilePictureExplainer, softWrap: true)),
  ],
);

Widget _sharedInformationList(
  BuildContext context,
  CircleDetailsState state,
) => Column(
  children: [
    ...
    if (state.profileInfo!.details.events.isNotEmpty)
      ...detailsList(
        context,
        state.profileInfo!.details.events.map(
          (label, date) => MapEntry(
            label,
            DateFormat.yMd(
        ...
        editCallback: (label, doShare) => updateSharedInformationWithCircle(
          state: state,
          detailSharingSettings: {...state.profileInfo!.sharingSettings.events},
          label: label,
          doShare: doShare,
          updateDetailSharingSettings: (sharingSettings) =>
              context.read<Storage<ProfileInfo>>().set(
                ...
                state.profileInfo!.copyWith(
                  sharingSettings: state.profileInfo!.sharingSettings.copyWith(
                    events: sharingSettings,
                  ),
                ),
              ),
        ),
      ),
    if (state.profileInfo!.details.misc.isNotEmpty)
      ...detailsList(
        context,
        state.profileInfo!.details.misc,
        circleId: state.circleId!,
        title: Text(
          'Misc',
          textScaler: const TextScaler.linear(1.4),
        ...
        getDetailSharingSettings: (l) =>
            state.profileInfo!.sharingSettings.misc[l],
        circles: state.circles,
        circleMemberships: state.circleMemberships,
        editCallback: (label, doShare) => updateSharedInformationWithCircle(
          state: state,
          detailSharingSettings: {...state.profileInfo!.sharingSettings.misc},
          label: label,
          doShare: doShare,
          updateDetailSharingSettings: (sharingSettings) =>
              context.read<Storage<ProfileInfo>>().set(
                ...
                state.profileInfo!.copyWith(
                  sharingSettings: state.profileInfo!.sharingSettings.copyWith(
                    misc: sharingSettings,
                  ),
                ),
              ),
        ),
      ),
    if (state.profileInfo!.details.tags.isNotEmpty)
      ...detailsList(
        context,
        state.profileInfo!.details.tags,
        circleId: state.circleId!,
        title: Text(
          'Tags',
          textScaler: const TextScaler.linear(1.4),
        ...
        hideLabel: true,
        getDetailSharingSettings: (l) =>
            state.profileInfo!.sharingSettings.tags[l],
        circles: state.circles,
        circleMemberships: state.circleMemberships,
        editCallback: (label, doShare) => updateSharedInformationWithCircle(
          state: state,
          detailSharingSettings: {...state.profileInfo!.sharingSettings.tags},
          label: label,
          doShare: doShare,
          updateDetailSharingSettings: (sharingSettings) =>
              context.read<Storage<ProfileInfo>>().set(
    ...
    if (state.profileInfo?.temporaryLocations.isNotEmpty ?? false)
      ..._card(
        ...
        filterTemporaryLocations(state.profileInfo!.temporaryLocations).entries
            .map(
              (l) => CheckboxListTile(
          ...
          actions: [
            IconButton(
              onPressed: () async {
                ...
                if (confirm == true) {
                  await context.read<CircleDetailsCubit>().removeCircle();
                  if (context.mounted) {
        ...
        children: [
              ...
              child: Row(
                children: [
                  ...
                  IconButton(
                    ...
                    icon: Icon(
                      (_topHeight == 0) ? Icons.expand_more : Icons.expand_less,
          ...
          AnimatedContainer(
            ...
            child: Padding(
              ...
              child:
                  (widget.state.circleMemberships.isEmpty &&
