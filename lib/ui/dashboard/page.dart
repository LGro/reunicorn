...
class _DashboardPageState extends State<DashboardPage> {
      ...
      body: BlocProvider(
        ...
        child: BlocConsumer<DashboardCubit, DashboardState>(
          ...
          builder: (context, state) =>
            ...
            final rowsArea =
                (totalH - 3 * headerH - 3 * buttonH).clamp(0, totalH);
            // Split equally between active row areas
            final numActive = ((introductions.isNotEmpty) ? 1 : 0) +
                ((closeByMatches.isNotEmpty) ? 1 : 0) +
  ...
  void _setIfChanged(
      {bool header = false, bool button = false, required double value}) {
    setState(() {
...
class _CloseByRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final start = formatDateTime(
        match.start, Localizations.localeOf(context).languageCode);
    final end =
