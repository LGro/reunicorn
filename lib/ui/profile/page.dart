                ...
                (circleId, circleLabel) => MapEntry<String, Widget>(
                  ...
                  Dismissible(
                    ...
                    child: GestureDetector(
                      ...
                      onTap: () async => pickCirclePicture(context, (p) async {
      ...
      <Widget>[
        ...details.entries.map((entry) {
          ...
          return Dismissible(
            ...
            child: GestureDetector(
              behavior: HitTestBehavior.opaque,
              onTap: (editCallback == null) ? null : () => editCallback(label),
              child: Padding(
                padding: const EdgeInsets.only(top: 8),
                child: Row(
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          if (circleNames != null && numSharedContacts != null)
                            Text(
                              textScaler: const TextScaler.linear(1.1),
  ...
  builder: (buildContext) => DraggableScrollableSheet(
    ...
    builder: (_, scrollController) => SingleChildScrollView(
      ...
      child: EditOrAddWidget(
        ...
        label: label,
        value: value,
        onDelete: () async => onDelete().then(
          (_) =>
              (buildContext.mounted) ? Navigator.of(buildContext).pop() : null,
        ),
        onAddOrSave: (newLabel, newValue, circlesWithSelection) async =>
          ...
          builder: (buildContext) => DraggableScrollableSheet(
            ...
            builder: (_, scrollController) => SingleChildScrollView(
              ...
              child: EditOrAddAddressWidget(
                ...
                circles: circles
                    .map(
                      (cId, cLabel) => MapEntry(cId, (
                        cId,
                        cLabel,
                        profileSharingSettings.addresses[label]?.contains(
                              cId,
                            ) ??
                            false,
                        circleMemberships.values
                ...
                onAddOrSave:
                    (oldLabel, label, value, circlesWithSelection) async {
                ...
                circles: circles
                    .map(
                      (cId, cLabel) => MapEntry(cId, (
                        cId,
                        cLabel,
                        profileSharingSettings.organizations[id]?.contains(
                              cId,
                            ) ??
                            false,
                        circleMemberships.values
