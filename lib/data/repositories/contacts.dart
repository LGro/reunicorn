...
class ContactsRepository {
  ...
  Future<void> initializeFromPersistentStorage() async {
    ...
    try {
      ...
      DebugLogger().log(
        'Error initializing circles from persistent storage:\n$e',
      );
    }
    ...
    try {
      ...
      DebugLogger().log(
        'Error initializing contacts from persistent storage:\n$e',
      );
    }
    ...
    try {
      ...
      DebugLogger().log(
        'Error initializing updates from persistent storage:\n$e',
      );
    }
    ...
    try {
      ...
      DebugLogger().log(
        'Error initializing batches from persistent storage:\n$e',
      );
    }
  }
  ...
  Future<void> _veilidUpdateValueChangeCallback(
    VeilidUpdateValueChange update,
  ) async {
  ...
  Future<bool> tryShareWithContactDHT(
  ...
  Future<bool> updateSharingDHT() async {
    ...
    await Future.wait([
      for (final contact in _contacts.values.where(
        (c) => c.sharedProfile != null,
      ))
  ...
  Future<bool> restore(
    ...
    // TODO: read record
    try {
      final jsonString = await distributedStorage.readBackupRecord(
        recordKey,
        secret,
      );
      if (jsonString == null) {
  ...
  Future<CoagContact> createContactForInvite(
    String name, {
    PublicKey? pubKey,
    bool awaitDhtSharingAttempt = false,
  }) async {
    // Create contact
    ...
    final dhtSharingAttempt = tryShareWithContactDHT(
  ...
  List<String> getContactIdsForCircle(String circleId) => _circleMemberships
      .map(
        (coagContactId, circleIds) => MapEntry(
          coagContactId,
          (circleIds.contains(circleId)) ? coagContactId : null,
        ),
      )
      .values
      .whereType<String>()
  ...
  Future<void> removeCircle(String circleId) async {
    ...
    _circleMemberships = _circleMemberships.map(
      (contactId, circleIds) =>
  ...
  Future<BatchInvite?> handleBatchInvite(
    ...
    if (profileInfo != null) {
      ...
      await setProfileInfo(
        profileInfo.copyWith(
          sharingSettings: profileInfo.sharingSettings.copyWith(
            names: updatedNameSharingSettings,
          ),
        ),
    ...
    await mySubkeyRecord.tryWriteBytes(
      utf8.encode(jsonEncode(mySubkeyContent.toJson())),
    );
  ...
  Future<MapEntry<String, Typed<FixedEncodedString43>>?>
    ...
    if (contact?.dhtSettings.recordKeyThemSharing == null &&
        contactSubkeyContent.records.containsKey(
          batch.myKeyPair.key.toString(),
        )) {
      contact = contact?.copyWith(
        dhtSettings: contact.dhtSettings.copyWith(
          recordKeyThemSharing:
              contactSubkeyContent.records[batch.myKeyPair.key.toString()],
        ),
      );
      if (contact != null) {
  ...
  Future<void> updateBatchInviteForContact(String coagContactId) async {
    ...
    for (final batchInvite in _batchInvites.values) {
      if (batchInvite.myConnectionRecords.containsKey(
        contact!.dhtSettings.theirPublicKey.toString(),
      )) {
  ...
  Future<String?> acceptIntroduction(
    ...
    final introducerDhtUpdate = tryShareWithContactDHT(
